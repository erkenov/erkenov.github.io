...
/**
 * /balance — client Billing dashboard (plan payment + usage balance).
 *
 * Embedded inside the white-labeled GoHighLevel sub-account as a Custom Menu
 * Link (iframe, menu item "Billing"). GHL passes the sub-account id via the
 * {{location.id}} merge field, so the link is e.g.
 * https://erken.systems/balance?locationId={{location.id}}
 *
 * Shows the three prepay plan tiers (Payoneer links), prepaid balance + usage
 * (from the agency ledger sheet) and live activity pulled from the client's
 * own GoHighLevel account. Iframe-friendly: no site nav/header, fills the
 * frame, own dark background.
 */
...
/** Payoneer open-amount top-up link (payer enters the sum; Shamil 2026-09-14).
 *  Opens with target _blank to escape the GHL iframe. Plan payments go
 *  through the stable /pay/* redirects — no per-plan tokens in this file. */
const TOP_UP_LINK =
  "https://link.payoneer.com/Token?t=A91129B0E6F54CE3B0F5244ACDA6D48A&src=dpl";

/** Prepay plan cards, derived from the single pricing source
 *  (src/lib/pricing.ts) so the numbers can't drift from the rest of the site. */
const PLANS = PLATFORM_BILLING_PERIODS.map((p) => {
  const { billedTotal, discountPct } = billingPeriodMath(p);
  return {
    name: p.label,
    total: `$${billedTotal.toLocaleString()}`,
    monthly: `$${p.perMonth}/mo`,
    note: discountPct > 0 ? `save ${discountPct}%` : "billed monthly",
    link: `/pay/${p.id}`,
    featured: p.id === "yearly",
  };
});
  ...
  return (
    ...
    >
      <div className="mx-auto" style={{ maxWidth: 720 }}>
        ...
        {/* Your plan — prepay tiers, fixed-amount Payoneer links.
            Renders immediately, independent of the balance API. */}
        <div className="text-xs mb-2" style={{ color: "rgba(232,239,233,0.5)" }}>Your plan</div>
          ...
          {PLANS.map((p) => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${p.featured ? "rgba(126,166,135,0.55)" : "rgba(255,255,255,0.08)"}`,
              }}
            >
              <div className="flex items-center justify-between mb-3">
                ...
                {p.featured && (
                  <span
                    ...
                    style={{ fontSize: 10, background: "rgba(126,166,135,0.2)", color: "#7ea687" }}
                  >
                    BEST VALUE
                  </span>
                )}
              ...
              <div
                ...
                style={{ fontSize: "clamp(24px,5vw,32px)", color: "#CDEBD2", letterSpacing: "-0.03em" }}
              >
                {p.total}
              </div>
              <div className="text-xs mt-1 mb-4" style={{ color: "rgba(232,239,233,0.55)" }}>
                {p.monthly} · {p.note}
              </div>
              <a
                href={p.link}
                ...
                className="mt-auto inline-block text-center rounded-lg px-4 py-2.5 text-sm font-semibold transition-transform hover:scale-[1.02]"
                style={
                  p.featured
                    ? { background: "#E89F1F", color: "#1a1405" }
                    : { background: "rgba(232,159,31,0.15)", color: "#E89F1F" }
                }
              >
                Pay {p.total} →
              </a>
        ...
        <div className="text-xs mb-2" style={{ color: "rgba(232,239,233,0.5)" }}>Usage balance</div>
